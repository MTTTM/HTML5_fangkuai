Quintus.Input = function(Q) {
    ...
    Q.InputSystem = Q.Evented.extend({
        ...
        bindKey: function(key, name) {
            Q.input.keys[KEY_NAMES || key] = name;
        },
        /*遍历事件并绑定fn*/
        ...
        touchControls: function(opts) {
            ...
            function touchDispatch(event) {
                ...
                //将Q。inputs数组中的所有条目设成false，目的是取消所有小键盘的输入标记
                //同时在wasOn对象中记下所有设置了标记的输入
                ...
                //遍历设备上当前发生的所有触摸，让后吧他们映射成小键盘的按压，并且设置适当的输入标记，
                //若之前未设置输入标记，该循环出发Q。input的事件，否则从wasOn数组中删除该动作
                console.log(event)
                ...
                //在touchDispatch被调用之前设置了标记的输入，让后出发适当的按钮松开事件
                for (actionName in wasOn) {
                    Q.input.trigger(actionName + "Up");
                }
                return null;
            }
            //事件变动的时候触发touchDispatch
        ...
        disableTouchControls: function() {
            Q.el.off("touchstart touchmove touchcancel");
