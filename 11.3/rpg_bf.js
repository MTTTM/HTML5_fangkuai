$(function() {
        ...
        move: function(dt) {
            ...
            if (p.wait <= 0) {
                ...
                if (p.attacking) {
                    ...
                    if (p.dx > 0) { destX += 1; } else if (p.dx < 0) { destX -= 1 };
                    if (p.dy > 0) { destY += 1; } else if (p.dy < 0) { destY -= 1; }
    ...
    Q.register('player_input', {
        ...
        input: function() {
            var p = this.entity.p;
            if (Q.inputs['left']) { p.dx = -1 } else if (Q.inputs['right']) { p.dx = 1; } else { p.dx = 0; }
            if (Q.inputs['up']) { p.dy = -1 } else if (Q.inputs['down']) { p.dy = 1; } else { p.dy = 0; }
        }
    });
    ...
    Q.Player = Q.Sprite.extend({
        ...
        attack: function(data) {
            ...
            data.sprite.trigger('interact', { source: this, damage: damage });
    ...
    //             data.source.trigger('interact', { source: this, damage: damage });
    //         }
    ...
    Q.load(['characters.png',
        ...
        Q.sheet('characters', 'characters.png', { tilew: tileSize, tileh: tileSize });

        Q.sheet('tiles', 'dungeon.png', { tilew: tileSize, tileh: tileSize });

        Q.sheet('items', 'items.png', { tilew: tileSize, tileh: tileSize });
